...
const LIST_ROOT = -777;      // синтетический корень для агрегированного плана «Список крафта» (см. js/list.js)
let listMode = false;        // открыт ли раздел «Список крафта» (для роутинга/навигации)
let cycle6Mode = false;      // открыт ли раздел Cycle 6 (для подсветки навигации шапки)
let craftList = [];          // [{id, qty}] предметы списка крафта (истина — URL-хеш #list=…, бэкап — localStorage)
...
const EN = {
  ...
  "Модульная постройка":"Modular Building",
...
function setLang(l){
  ...
  applyStatic();
  if(!DATA) return;
  renderCats(); updateViewBtns(); renderItems(); renderCrumbs(); updateListNav();
  const h=(location.hash||"").replace("#","");
  if(listMode || /^list/.test(h)) showList();
  else if(h==="ships") shipsCompare();
